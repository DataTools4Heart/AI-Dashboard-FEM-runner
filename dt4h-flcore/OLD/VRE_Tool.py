    ...
    def run_my_demo_pipeline(self, input_files, output_file_path):
        ...
        try:
            ...
            return dt4h_demonstrator(
                api_prefix: self.configuration.get('api_prefix'),
                access_token: self.configuration.get('token'),
                server_node: self.configuration.get('server_node_list').split(","),
                client_node_list: self.configuration.get('client_node_list').split(","),
                input_params: self.configuration.get('input_params', {}),
                tool_id: self.configuration.get('tool_id', 'flcore'),
                tool_name: self.configuration.get('tool_name', 'fLcore'),
                health_check: self.configuration.get('health_check', False)
            )
