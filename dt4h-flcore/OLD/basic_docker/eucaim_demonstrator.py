...
def second_demonstrator( token:str, node_list:list[str], tool:str, queue:Queue ) -> dict:
    ...
    if step_one[ 'status' ] != 'success':
        ...
        #queue.put({'status': 'failure', 'message': f"Failed to run \"execture_tool\" on nodes { node_list } with tool \"{ tool_name }\"." })
    else:
